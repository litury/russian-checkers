...
describe('computeFieldLayout baseline budget', () => {
	it('retains the portrait board budget', () => {
	...
	it('retains the wide desktop board budget', () => {
	...
	it('bounds the original budget for short portrait screens', () => {
		const field = computeFieldLayout(390, 400);
		expect(field.fieldSize).toBe(
			400 - (layout.hudBar + 24) - layout.boardBottomGap,
		);

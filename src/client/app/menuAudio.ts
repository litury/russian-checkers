...
export function createMenuAudio(sdk:IYandexSdk) {
 ...
 const say=(name:string)=>{
  if(voice){try{voice.stop()}catch{}effects.delete(voice);voice=undefined;duckMatch(false);}
  duckMatch(true);
  ...
  effects.add(s);s.onended=()=>{duckMatch(false);effects.delete(s);s.disconnect();g.disconnect();if(voice===s)voice=undefined;};
  voice=s;s.start();
 };

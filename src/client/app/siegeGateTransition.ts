...
export function siegeGatePose(ms: number, distance: number, duration = gateDurationMs) {
 const t = Math.max(0, Math.min(1, ms / duration));
 ...
 */
export function animateSiegeGates(root: HTMLElement, duration = gateDurationMs) {
 // Compressed repeats keep every phase proportional to the canonical pass.
 const scale = duration / gateDurationMs;
 const titleMs = titleDepartureMs * scale;
 ...
 try {
  ...
  for (const {nodes, x} of groups) for (const node of nodes) {
   animations.push(node.animate(
    ...
    {delay:titleMs, duration:duration-titleMs, easing:'cubic-bezier(0.3333333333,0,0.6666666667,1)', fill:'both'},
   ));
  }
  ...
  if (canopy) animations.push(canopy.animate(
   ...
   {duration:titleMs, easing:'ease-in', fill:'both'},
  ));
